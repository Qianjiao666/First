...
本文档按公开版本号的实际演进顺序记录项目变化。公开版本号只使用两段式 `vX.Y`。
...
- 上线任务协作增强：任务广场筛选、任务详情下一步、我的任务分组操作、补充交付和管理员仲裁弹窗。
- 同步补齐 Supabase RPC `record_task_supplement` 与 `task-complete` Edge Function 的补充交付处理，并完成腾讯云 CVM 静态切换。
