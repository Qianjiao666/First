...
create policy task_attachments_read_visible on public.task_attachments
  for select to anon, authenticated using (public.can_access_task_storage(task_id, (select auth.uid())));
...
create policy task_activity_log_read_visible on public.task_activity_log
  for select to anon, authenticated using (
    public.can_access_task_storage(task_id, (select auth.uid()))
  );
...
create policy task_attachments_storage_read on storage.objects
  for select to anon, authenticated using (
    ...
    and public.can_access_task_storage(((storage.foldername(name))[2])::uuid, (select auth.uid()))
  );
