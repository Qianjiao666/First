...
test("homepage exposes v1.6 consistently in its changelog entry point", async () => {
  const html = await read("index.html");
  assert.match(html, /id="mkj-log-button"[^>]*title="查看产品更新日志（当前 v1\.6）"/);
  assert.match(html, /id="mkj-log-button"[^>]*aria-label="查看产品更新日志（当前 v1\.6）"/);
  assert.match(html, /<b>更新日志 · v1\.6<\/b>/);
  const entries = [...html.matchAll(/<article class="mkj-changelog-entry"><time>[^<]+ · (v\d+\.\d+)<\/time>/g)].map((match) => match[1]);
  assert.equal(entries[0], "v1.6");
});

test("CHANGELOG is ordered newest to oldest and starts at v1.6", async () => {
  const changelog = await read("CHANGELOG.md");
  const versions = [...changelog.matchAll(/^## (v\d+\.\d+)$/gm)].map((match) => match[1]);
  assert.deepEqual(versions, ["v1.6","v1.5","v1.4","v1.3","v1.2","v1.1","v1.0","v0.9","v0.8","v0.7","v0.6","v0.5","v0.4","v0.3","v0.2","v0.1"]);
});
