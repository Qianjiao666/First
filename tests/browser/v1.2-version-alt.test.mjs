...
test("homepage exposes v1.3 consistently in its changelog entry point", async () => {
  const html = await read("index.html");
  assert.match(html, /id="mkj-log-button"[^>]*title="查看产品更新日志（当前 v1\.3）"/);
  assert.match(html, /id="mkj-log-button"[^>]*aria-label="查看产品更新日志（当前 v1\.3）"/);
  assert.match(html, /<b>更新日志 · v1\.3<\/b>/);
  const entries = [...html.matchAll(/<article class="mkj-changelog-entry"><time>[^<]+ · (v\d+\.\d+)<\/time>/g)].map((match) => match[1]);
  assert.equal(entries[0], "v1.3");
});

test("CHANGELOG is ordered newest to oldest and starts at v1.3", async () => {
